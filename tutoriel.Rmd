...
Voici la liste des librairies que nous allons utiliser pour l'atelier. Assurez-vous qu'elles soient téléchargées.
...
Les modèles que nous construirons durant l'atelier peuvent prendre un certain à estimer les paramètres. Après avoir téléchargé les dossiers de l'atelier (dans ce dépôt GitHub)[https://github.com/quantitative-ecologist/Atelier-CSL-EcotoQ], vous pouvez effectuer ces commandes pour les importer dans votre session:
...
- `result_conc_mean`: la concentration efficace moyenne en mg/L
- `result_conc_mean_log`: le log de la concentration efficace moyenne
- `result_conc1_mean_binary`: si la concentration est moins toxique (`0`) ou plus toxique (`1`)
...
## Distribution de la concentration efficace

Commençons par évaluer la distribution de la variable réponse, qui est le log de la concentration efficace moyenne. Nous allons faire nos graphiques avec la librairie `ggplot2`.
...
## Concentration efficace selon l'espèce

Vérifions si les concentrations efficaces varient selon l'espèce.
...
## Concentration efficace selon l'exposition

Finalement, on peut aussi se douter que la concentration efficace pourrait changer selon le type d'exposition.
...
Les vérifications nous indiquent qu'il semble y avoir une structure hiérarchique dans nos données. En termes simples, cela signifie qu'il y a de la variation dans les concentrations efficaces entre les différents groupes que nous avons vu, notamment, entre les espèces.
...
Cette situation est particulièrement commune lorsqu'on analyse des données biologiques, qui ont tendance à suivre d'autres types de distribution telles que Poisson ou binomiale.
...
- $Y$: la concentration efficace est 1 (plus toxique) ou 0 (moins toxique)
- $X_1$: le type d'exposition
- $X_2$: la température moyenne du medium degrés Celsius
- $X_3$: la longévité de l'espèce en jours

Si on modélisait la relation entre la toxicité de la concentration efficace et la température du medium avec une régression linéaire classique, on obtiendrait ceci:
...
On peut voir à gauche que les résidus de notre modèle ne respectent pas la condition de normalité et d'homoscédasticité.

Pour ces raisons, on doit donc établir une limite inférieure et supérieure à nos données en **spécifiant une distribution de Bernoulli** pour modéliser la toxicité de la concentration efficace.
...
Dans cet exemple, on s'intéresse à estimer comment la toxicité de la concentration efficace varie selon le type d'exposition, la longévité moyenne de l'espèce, et la température du médium durant l'expérience.
...
$p$ est la probabilité que la concentration efficace soit toxique (1) pour chaque observation. 
...
Une autre étape d'inspection de convergence des chaînes de Markov est d'évaluer les **$\hat{R}$** et les **tailles d'échantillon efficace** pour chaque paramètre d'intérêt.

- Dans @Burkner2017, on peut lire que la taille d'échantillon efficace est: "le nombre d'échantillons indépendants de la distribution postérieure qui serait attendu pour produire la même erreur standard de la moyenne postérieure que celle obtenue à partir des échantillons dépendants retournés par l'algorithme MCMC". Dans `brms`, le résumé du modèle fournit une estimation des tailles d'échantillon efficaces de la partie centrale et des queues de la distribution postérieure. Le `Bulk_ESS` rapporte l'efficacité de l'échantillonnage de la partie centrale de la distribution postérieure (c'est-à-dire la médiane ou la moyenne), tandis que le `Tail_ESS` diagnostique l'efficacité de l'échantillonnage des queues de la distribution postérieure (c'est-à-dire les quantiles 5% et 95%). Nous utilisons un seuil de <100 comme règle de décision pour savoir si les chaînes ont convergé pour les paramètres [@Vehtari.etal2021a].
...
- La probabilité que la CE50 soit plus toxique augmente avec la longévité
- La probabilité que la CE50 soit plus toxique diminue avec la température moyenne
...
Ici, on va utiliser les distributions postérieures des moyennes prédites de chaque type d'exposition. Nous avons déjà extrait ceci dans une étape ultérieure et assigné à l'objet `params1`. 
...
1. Il y a 40% de chances que la concentration efficace soit plus toxique lorsque les conditions expérimentales utilisent un flux d'eau continu. Nous avons de bonnes preuves que ce pourcentage ne diffère pas entre un traitement avec renouvellement. Nous avons également de fortes preuves que des conditions statiques augmentent les changes que la concentration efficace soit toxique par rapport à un flux continu. Les expériences où le traitement n'est pas spécifié ont les plus hautes chances que la concentration efficace soit toxique. 

2. Nous avons de fortes preuves que la probabilité que la concentration efficace soit toxique diminue avec la température du médium. Les chances que la concentration soit toxique diminuent d'environ 25% des plus basses aux plus hautes températures.

3. Nous avons de fortes preuves que la probabilité que la concentration efficace soit toxique augmente avec la longévité moyenne de l'espèce. À partir des espèces à courte durée de vie, on observe une augmentation d'environ 10% à 15% dans la probabilité pour les espèces à plus haute longévité.

# 8. Théorie: les HGLM

## Complexité des données biologiques

En explorant nos données, nous avons vu qu'il semble y avoir une structure hiérarchique, où la concentration efficace varie entre les espèces.
...
Reprenons l'exemple de la CE50 en fonction de la température du médium. Cette fois, on utilise le log de la concentration en mg/L pour mieux voir la distribution des points, et on ajoute les espèces dans la figure. Ici, chaque couleur correspond à un taxon différent.
...
1. Est-ce que les taxons diffèrent dans leur concentration efficace moyenne?
2. Est-ce que la relation entre la concentration efficace et la température change selon les taxons?
...
On s'intéresse toujours à estimer comment la concentration efficace ($Y$) varie selon le type d'exposition, la longévité moyenne de l'espèce, et la température du médium durant l'expérience ($X$). On ajoute toutefois **les espèces comme effet aléatoire** ($U$).

Ici, on veut donc savoir si les espèces varient dans leur concentration efficace moyenne, et si la relation entre celle-ci et la température change d'une espèce à l'autre.
...
Pour mieux visualiser les relations le long de la distribution des points, nous ferons cette fois-ci un HLM en utilisant le log de la concentration efficace moyenne. Notre modèle devra donc respecter les suppositions de base des modèles linéaires.
...
Nous verrons plus tard comment cette matrice de covariance nous permet de savoir si la concentration efficace moyenne des espèces est associée à leur réponse à la température du médium.
...
Dans le contexte des modèles hiérarchiques, il est pertinent de décomposer la variance pour évaluer dans quelle mesure les effets fixes et les effets aléatoires contribuent à expliquer de la variation dans $Y$.

Je vous conseille de lire l'article de [Nakagawa et Schielzeth (2012)]((https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/j.2041-210x.2012.00261.x)) pour une revue de la théorie.

Voici les deux mesures que nous allons vérifier:

$$
R_{marginal}^{2} = \frac{V_{fixe}}{V_{résiduelle} + V_{fixe} + V_{aléatoire}}
$$

$$
R_{conditionnel}^{2} = \frac{V_{fixe} + V_{aléatoire}}{V_{résiduelle} + V_{fixe} + V_{aléatoire}}
$$

```{r r2, cache=TRUE}
performance::r2_nakagawa(hglm)
```

- La température, la longévité, et le type d'exposition ensemble n'expliquent que 0.4% de la variation dans la CE50, ce qui est très faible.

- Le $R_{conditionnel}^{2}$ monte à 30.8%. Si on soustrait le $R_{conditionnel}^{2}$ et le $R_{marginal}^{2}$, celà indique que 30.8% de la variation dans la CE50 est dûe à des différences entre les espèces, ce qui est très grand!

### Figures

#### Prédictions des effets fixes

On répète les mêmes étapes que le GLM pour calculer les prédictions de la CE50 en fonction de la température et de la longévité moyenne.
...
**Calcul des prédictions par espèce**
...
**Extraction des paramètres par espèce**

Ici, nous allons extraire les pentes et les ordonnées à l'origine individuellement pour chaque espèce afin de visualiser la corrélation entre les deux paramètres.

On commence par extraire les distributions postérieures dans une `data.table`.

```{r}
# Transformer en data.table pour manipuler
paramsdt3 <- data.table(params2)

# Changer en format long
ranefs <- melt(
  data = paramsdt3,
  variable.name = "parameter",
  measure = patterns("^r_tax_gs")
)[, .(parameter, value)]
```

On réassigne ensuite le nom de l'espèce et le nom des paramètres dans des colones individuelles.

```{r}
# Réassigner les noms d'espèces
tax_gs <- gsub("^r_tax_gs\\[([^,]+).*", "\\1", ranefs$parameter)
ranefs[
  , tax_gs := tax_gs
]

# Assigner les paramètres à une colone
ranefs[parameter %like% "Intercept", parameter_type := "intercept"]
ranefs[is.na(parameter_type), parameter_type := "slope"]
```

Finalement, on calcule les médianes par espèce pour la pente et l'ordonnée à l'origine.

```{r}
# Calculer médianes
ranefs[
  , mediane := median(value),
  by = .(tax_gs, parameter_type)
]

# Table unique pour tous les paramètres
ranefs1 <- unique(ranefs[, .(parameter_type, mediane, tax_gs)])

# Ramener en format large
ranefs2 <- dcast(
  data = ranefs1,
  value.var = "mediane",
  formula = tax_gs ~ parameter_type
)
```

#### Visualisation

On peut maintenant produire les graphiques. 

**Effets fixes**

À titre de rappel, on a la même pente pour l'ensemble des types d'exposition, et des ordonnées à l'origine qui diffèrent (c-à-d, les moyennes de concentration diffèrent). Ici, la droite correspond au type d'exposition `F`.
...
ggplot(
  ...
  #scale_x_continuous(breaks = seq(0.25, 1, 0.25), limits = c(0.25, 1)) +
  xlab("\nlog(concentration efficace (mg/L))") +
  ylab("Densité\n") +
  labs(fill = "Exposition", colour = "Exposition") +
  theme_classic(base_size = 14) +
  theme(legend.position = "top")
...
hglm_p1 <- ggplot() +
  ...
  ylab("log(concentration efficace (mg/L))\n") +
  scale_y_continuous(
    breaks = seq(-8, 8, 4),
    limits = c(-10, 8)
  ) +
...
Finalement, nous avons les droites pour chaque espèce. Ici, on ne montre que les droites pour les espèces qui ont au moins 10 observations de température.
...
```{r plot24, fig.height=6, fig.width=7, cache = TRUE}
p <- ggplot(
  ranefs2,
  aes(
    x = intercept,
  ...
  geom_point(aes(colour = tax_gs)) +
  geom_smooth(method = lm, se = FALSE, colour = "black") +
  scale_color_viridis(discrete = TRUE, option = "D") +
  ylab("Pente") +
  xlab("Ordonnée à l'origine") +
  scale_x_continuous(breaks = seq(-2, 2, 1), limits = c(-2.3, 2)) +
  scale_y_continuous(breaks = seq(-2, 2, 1), limits = c(-2.3, 2)) +
  theme_classic(base_size = 14) +
  theme(legend.position = "none")
...
Les résultats semblent avoir **beaucoup changé**:

- on ne détecte plus de différences entre les types d'exposition
- la relation entre la concentration et la température est presque nulle
- la relation entre la concentration et la température est presque nulle

Mais n'ayez crainte, il y a **quelques explications potentielles**.

**Premièrement**, le fait de transformer une variable continue en variable binaire change sa nature. Ici, les auteurs ont considéré des valeurs de CE50 (mg/L)dans l'intervalle $[10^0, -\infty]$ comme toxique (1), et $[10^0, \infty]$ comme moins toxique (0). Ceci créé un débalancement dans les classes. On voit dans l'article de Schür (2023) qu'il y a davantage de classes non toxiques que toxiques.

**Deuxièmement**, il y a une forte imbalance entre les types d'exposition, et de transformer la CE50 en variable binaire pourrait aussi avoir joué sur les résultats en ajoutant encore plus d'imbalance à l'intérieur types d'exposition.

### Effets aléatoires

On observe:  

- d'importantes différences entre les espèces dans la CE50 moyenne 
- d'importantes différences entre les espèces dans la réponse à la température
- qu'il n'y a pas de patron évident entre la sensibilité moyenne à la CE50 et la réponse à la température

Ainsi, il est clair que la CE50 diffère d'une espèce à l'autre, ce qui est important à considérer dans un contexte écotoxicologique. Celles-ci ne réagissent pas de la même façon à la température. Pour certaines, une augmentation mène à une CE50 plus élevée, alors que pour d'autres, le patron est inversé.

Finalement, ces différences entre espèces sont beaucoup plus importantes et contribuent à expliquer davantage de variation dans la CE50 que le type d'exposition durant l'expérience.
