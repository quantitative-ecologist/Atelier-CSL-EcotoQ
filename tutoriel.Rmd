...
```{r fig.height=4, fig.width=10, echo = FALSE, cache = TRUE}
...
```{r, echo = FALSE, fig.height=4, fig.width=10, cache=TRUE}
...
## Exemple avec les données ADORE
...
Si on essayait modéliser la relation entre la concentration effective (plus toxique ou moins toxique) et la température du medium, on obtiendrait ceci :

```{r plot6, fig.height=4, fig.width=9, cache=TRUE, echo=FALSE}
fit2 <- lm(
...
De plus, on peut voir à gauche que les résidus de notre modèle ne satisfont pas la condition d'homoscédasticité.

Pour ces raisons, on doit donc établir une limite inférieure et supérieure à nos données en spécifiant une distribution de Bernoulli pour modéliser nos données.

### Construction du modèle

Nous allons donc construire un GLM binomial Bayésien en utilisant les variables spécifiées ci-haut.
...
Voici la formule de notre modèle que nous spécifions.
...
ggplot(
  data = tab1,
  aes(x = Zmedia_temperature_mean, y = estimate__)
) +
  geom_point(
    data = anim,
    aes(
  ...
  geom_ribbon(
    ...
    alpha = 0.20, fill = "#440154"
  ) +
  geom_line(linewidth = 1, alpha = 0.5, colour = "#440154") +
  ylab("Toxicité\n") +
  scale_y_continuous(
    breaks = seq(0, 1, 0.25),
    limits = c(0, 1)
