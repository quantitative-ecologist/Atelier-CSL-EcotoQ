...
Comme nous l'avons vu dans la section sur l'inférence Bayésienne, la distributions postérieures sont estimées en multipliant la distribution à priori et la distribution de vraisemblance des données (les preuves).
...
Les priors à spécifier sont :

- sur l'ordonnée à l'origine globale (la moyenne du type d'exposition `F`)
- les différences de moyenne par rapport à `F`
- les pentes pour la température et la longévité

Pour vous aider à comprendre, imaginez que vous savez avec la littérature que la dose effective devrait être plus toxique avec une augmentation de la température. Ainsi, pour la pente de ce paramètre, vous auriez pu spécifier une distribution Gaussienne avec une moyenne de 1 et un écart-type de 0.5. Vous diriez donc à votre modèle que vous assumez que cette valeur devrait être positive, se situant autour de 1, avec peu de variation autour de cette valeur.
...
À titre de rappel, `brms` utilise l'algorithme de MCMC NUTS pour estimer les paramètres du modèle.
...
Voici les spécifications pour l'estimation des paramètres par MCMC NUTS. Pour chacune des 4 chaînes, nous aurons :
